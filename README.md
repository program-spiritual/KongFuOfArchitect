...
差异化在符合人们的认知方式的体系中，占据着主导地位。如果将一个三维的立方体表示在一个二维的平面上，
我们可能丢失一部分信息，而我们如果能够立方体切开，并记录下来足够多的投影信息，那么在还原它的时候，我们能够保证一定的准确性。
假设我们将投影这种技术记为一种变换 `F` ，那么我们实际上就是通过 `F` 来转换并存储足够多的信息，而原来的球体信息就是我们的输入 `I` 。
那么对于任何一个投影，我们都可以表示为 `S(投)  = F(I) `, 假设我们要考虑到额外的属性 `V`，例如来自立方体宽高这种属性，我们可以将其
