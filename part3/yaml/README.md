...
对于集合，`YAML` 包括使用显式的指示符而不是使用缩进来表示空格的样式流。
集合中的流程顺序用方括号 `[]` 括起来的逗号分隔列表表示。
...
`YAML` 使用意为（`|`）的字面量类型的格式块编写，它表示换行计数。
