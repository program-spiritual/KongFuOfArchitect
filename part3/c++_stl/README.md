...
标准的 `C++` 库提供了一个字符串类类型，它支持上述所有操作，此外还具有更多功能。
让我们查看以下示例-
...
`C++`标准库提供了广泛的输入/输出功能集，我们将在后续章节中看到。
