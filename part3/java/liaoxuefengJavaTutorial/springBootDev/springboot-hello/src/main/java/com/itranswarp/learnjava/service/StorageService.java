...
public interface StorageService {
  ...
  public default void init() {
    logger.info(
      "Load configuration: root-dir = {}",
      storageConfig.get().getRootDir()
    );
    logger.info(
      "Load configuration: max-size = {}",
      storageConfig.get().getMaxSize()
    );
    logger.info(
      "Load configuration: allowed-types = {}",
      storageConfig.get().getAllowTypes()
    );
  }
}
